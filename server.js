...
function health(){
 ...
 const p=providerStatus(),beat=p?.at?Date.parse(p.at):0,workerAlive=Boolean(beat&&Date.now()-beat<180000),today=new Intl.DateTimeFormat('en-CA',{timeZone:TIMEZONE,year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date()),rendered=Number(db.prepare("SELECT COUNT(*) n FROM factory_generations WHERE day=? AND status IN ('review','completed') AND COALESCE(generationKind,'automatic')<>'review_retry'").get(today)?.n||0),approved=Number(metaGet('flow:approvedSubmits:'+today,'0')||0),override=(String(process.env.PUBLISHER_APPROVALS_OVERRIDE_DAY||'')===today?Number(process.env.PUBLISHER_APPROVALS_OVERRIDE_COUNT||0):0),completed=Math.max(rendered,approved,Number.isFinite(override)?override:0),current=db.prepare("SELECT episode,status,error,lastProgressAt FROM factory_items WHERE status IN ('generating','draft','regen_wait') ORDER BY CASE status WHEN 'generating' THEN 0 ELSE 1 END,episode LIMIT 1").get();
