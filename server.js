...
function health(){
 ...
 const publicationCopy=db.prepare("SELECT episode,title,description,status,scheduledAt,uploadAt,attempts,retryAt,error,videoId,filePath,remotePrivacyStatus,remotePublishAt,aiDisclosureSyncedAt,updatedAt FROM publication_items WHERE status NOT IN ('cancelled','deleted') ORDER BY episode LIMIT 20").all().map(x=>({episode:Number(x.episode),title:String(x.title||''),description:String(x.description||''),status:String(x.status||''),scheduled_at:x.scheduledAt||null,upload_at:x.uploadAt||null,attempts:Number(x.attempts||0),retry_at:Number(x.retryAt||0)||null,error:x.error?String(x.error).slice(0,800):null,video_id:x.videoId||null,has_file:Boolean(x.filePath),remote_privacy_status:x.remotePrivacyStatus||null,remote_publish_at:x.remotePublishAt||null,ai_disclosure_synced_at:x.aiDisclosureSyncedAt||null,updated_at:x.updatedAt||null}));
