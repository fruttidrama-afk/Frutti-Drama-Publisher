...
- FLOW-ERR-018 NO_CHARGE_RETRY_STORM — repeated automatic submissions continue after Google Flow explicitly reports unusual activity and confirms no charge, risking provider throttling and permanently blocking the serial head-of-line. Recovery: preserve the same episode, do not click Flow's failed-tile Retry inside the same attempt, record a per-episode no-charge streak, use short backoff only for the first transient failures, then after persistent repeated rejections enter a provider-wide 24-hour quarantine, clear non-cookie browser caches once, never allow another draft/REDO to bypass that quarantine, and reset the streak only after hard generation-start evidence.
