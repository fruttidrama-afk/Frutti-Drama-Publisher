...
- every recovered Flow video is now bound to a stable Flow asset identity derived from the tile/media DOM; the runtime stores that identity permanently, rejects any asset ID already recovered before, re-verifies the same identity immediately before opening the tile, and also rejects byte-identical or previously quarantined wrong MP4s. Fixed-size/virtualized Flow grids are diffed as multisets instead of assuming tile #0 is newest;
