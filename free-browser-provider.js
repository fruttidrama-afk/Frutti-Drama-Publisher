...
async function configureFlow(page){
  ...
  return{label:label||'settings-applied',mode:'Video',ratio:ASPECT_RATIO,model:currentModel,resolution:RESOLUTION_INTENT,duration:DURATION_LABEL,count:OUTPUT_LABEL,ingredients:CONFIG.characters.length>0};
}
...
,'i')).last();if(await ratio.count().catch(()=>0)&&await ratio.isVisible().catch(()=>false))return true;
    return false;
  };
  if(await visibleSetting())return;
  const b=await settingsButton(page);await b.click();const deadline=Date.now()+6500;
  while(Date.now()<deadline){if(await visibleSetting())return;await sleep(180)}
  throw new Error('FLOW_SETTINGS_MENU_NOT_OPEN:'+compact(await b.innerText().catch(()=>''),180));
}
async function clickRadio(page,re,label){
