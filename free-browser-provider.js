...
async function openLatestExpectedVideoTile(page,baselineInv){
  ...
  if(visible.length<expected)return{ready:false,opened:false,signal:`video-tile-count:${visible.length}<${expected}`};
  ...
  if(d)return{ready:true,opened:true,signal:`latest-expected-video-tile:${visible.length}`,index:0};
  await page.keyboard.press('Escape').catch(()=>{});
  return{ready:false,opened:false,signal:`latest-video-tile-no-download:${visible.length}`};
}
