...
,'i'),DURATION_LABEL);durationApplied=DURATION_LABEL}
  catch(e){if(!String(e?.message||e).startsWith('FLOW_SETTING_NOT_FOUND:'))throw e;publish('FLOW_SETTING_DEFAULT',{setting:'duration',requested:DURATION_LABEL,message:'Duration control is not exposed by this Flow model; exact duration remains enforced in the generation prompt.'})}
  let outputApplied='default';
  try{await clickRadio(page,new RegExp('^'+escapeRe(OUTPUT_LABEL)+'
    const ingredients=page.getByRole('radio',{name:/Ingredients/i}).last();
    if(await ingredients.count().catch(()=>0)&&await ingredients.isVisible().catch(()=>false)){
      if((await ingredients.getAttribute('aria-checked').catch(()=>null))!=='true')await clickInteractive(ingredients);
      await sleep(300);
...
setTimeout(()=>{void runProvider()},12000);
setInterval(()=>{void runProvider()},20*1000).unref();
,'i'),OUTPUT_LABEL);outputApplied=OUTPUT_LABEL}
  catch(e){if(!String(e?.message||e).startsWith('FLOW_SETTING_NOT_FOUND:'))throw e;publish('FLOW_SETTING_DEFAULT',{setting:'output_count',requested:OUTPUT_LABEL,message:'Output-count control is not exposed by this Flow model; keeping the model default.'})}
