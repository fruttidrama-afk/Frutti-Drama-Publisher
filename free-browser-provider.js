...
async function runProvider(){
  ...
  const retryAt=Date.now()+30000;
  db.prepare("UPDATE factory_items SET status='draft',providerRunId=NULL,runtimeAttemptCount=?,lastProgressAt=?,error=NULL,nextTry=?,updatedAt=? WHERE id=?").run(attempts,now(),retryAt,now(),fresh.id);
  setLifecycle(db,fresh,'RECONCILED_NO_GENERATION',{prior_generation_id:run,reconciled_at:now(),evidence:'Exact project returned to baseline with no busy state, no playable video and no fresh tile for 20+ minutes.',retry_at:new Date(retryAt).toISOString(),automatic_submit_forbidden:false});
  publish('NO_RETAINED_RENDER_RELEASED',{episode:'E'+fresh.episode,job_id:fresh.id,message:'No retained Flow asset after 20+ minutes; one clean serial retry is authorized.'});
