...
async function scrollRemote({x=VIEW.width/2,y=VIEW.height/2,deltaX=0,deltaY=0}={}){
 await start();
 const b=await page();
 try{
   const p=b.page;if(!p)throw new Error('No hay una pestaña activa para desplazar.');
   const px=Math.max(0,Math.min(VIEW.width-1,Number(x)||VIEW.width/2));
   const py=Math.max(0,Math.min(VIEW.height-1,Number(y)||VIEW.height/2));
   const dx=Math.max(-2400,Math.min(2400,Number(deltaX)||0));
   const dy=Math.max(-2400,Math.min(2400,Number(deltaY)||0));
   await p.mouse.move(px,py);
   await p.mouse.wheel(dx,dy);
   await sleep(140);
   return{ok:true,x:px,y:py,deltaX:dx,deltaY:dy};
...
function pageHtml(message='',error=''){
 const running=Boolean(pids().length&&windowId()),title=running?windowTitle():'',verified=verifiedInfo(),brand=CONFIG.branding||{},theme=brand.theme||{},show=String(CONFIG.identity?.show_name||CONFIG.identity?.publisher_name||'Publisher');
...
:root{--navy:${h(theme.primary||"#0d3152")};--gold:${h(theme.accent||theme.secondary||"#b59a64")};--ink:${h(theme.text||"#1d1d1b")};--muted:#6e6a63;--bg:${h(theme.background||"#f7f6f2")};--surface:${h(theme.surface||"#ffffff")};--line:#ded9cf;--ok:#315d35;--okbg:#eef4ed;--danger:#8c3c33}
*{box-sizing:border-box}body{margin:0;background:${brand.reference_image_url?`linear-gradient(180deg,color-mix(in srgb,var(--surface) 82%,transparent),color-mix(in srgb,var(--bg) 96%,transparent)),url("${h(brand.reference_image_url)}")`:`var(--bg)`};background-size:cover;background-position:center;background-attachment:fixed;color:var(--ink);font-family:Arial,Helvetica,sans-serif;padding:calc(18px + env(safe-area-inset-top)) 16px calc(34px + env(safe-area-inset-bottom));-webkit-font-smoothing:antialiased}.wrap{max-width:1080px;margin:auto}.brandhead{display:flex;align-items:center;gap:14px;margin:0 0 18px;padding:12px 14px;background:color-mix(in srgb,var(--surface) 92%,transparent);border:1px solid var(--line);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px)}.brandhead img{width:64px;height:64px;object-fit:contain;border-radius:16px;background:var(--surface)}.brandhead strong{font:400 27px Georgia,serif;color:var(--navy)}.brandref{width:100%;height:clamp(170px,28vw,300px);object-fit:cover;border:1px solid var(--line);border-radius:22px;margin:0 0 18px;box-shadow:0 18px 54px color-mix(in srgb,var(--navy) 10%,transparent)}.back{display:inline-flex;color:var(--navy);text-decoration:none;font-weight:800;margin-bottom:18px}.eyebrow{font-size:11px;letter-spacing:.2em;color:var(--gold);font-weight:800}h1{font:400 clamp(42px,7vw,64px)/.95 Georgia,serif;margin:10px 0 14px}.lead{font-size:17px;line-height:1.5;color:#47433d;max-width:790px}.steps{display:grid;grid-template-columns:repeat(5,1fr);gap:8px;margin:22px 0}.step{background:color-mix(in srgb,var(--surface) 94%,transparent);border:1px solid var(--line);padding:14px;min-height:118px;backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px)}.step b{display:block;color:var(--navy);margin:7px 0}.num{width:30px;height:30px;border-radius:50%;background:var(--navy);color:#fff;display:grid;place-items:center;font-weight:800}.step span{font-size:12px;color:var(--muted);line-height:1.4}.workspace{background:color-mix(in srgb,var(--surface) 95%,transparent);border:1px solid var(--line);padding:16px;backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px)}.bar{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:0 0 12px}.btn,button,input{font:inherit;min-height:48px;padding:10px 13px;border:1px solid #cfc8ba;background:var(--surface)}.btn,button{font-weight:800;cursor:pointer}.primary{background:var(--navy);color:#fff;border-color:var(--navy)}button:active{transform:scale(.97)}form{margin:0}.notice{padding:12px 13px;margin:10px 0;font-size:13px;line-height:1.45}.notice.ok{background:var(--okbg);border:1px solid #a9bea4;color:var(--ok)}.notice.err{background:#fff1ee;border:1px solid #d8aaa2;color:var(--danger)}.tip{background:#faf9f6;border-left:3px solid var(--gold);padding:11px 13px;color:var(--muted);font-size:13px;line-height:1.45;margin:11px 0}.screen-wrap{width:100%;overflow:auto;border:1px solid var(--line);background:#ececeb;margin-top:12px;-webkit-overflow-scrolling:touch}.screen-input{display:block;width:1024px;height:700px;border:0;padding:0;margin:0;background:#ececeb}.screen-note{font-size:12px;color:var(--muted);margin:8px 0 0}.remote-frame{width:100%;height:min(84vh,920px);min-height:560px;border:1px solid var(--line);background:#ececeb;margin-top:12px}.controller-tools{display:flex;gap:8px;flex-wrap:wrap;margin:10px 0}.controller-tools .btn{display:inline-flex;align-items:center;justify-content:center;text-decoration:none}.controller-help{font-size:12px;color:var(--muted);line-height:1.45}.live-note{background:#eef4ed;border:1px solid #a9bea4;color:#315d35;padding:11px 13px;font-size:13px;line-height:1.45;margin:10px 0}.entry{display:grid;grid-template-columns:1fr auto;gap:8px;margin:12px 0}.entry input{width:100%}.keys{display:flex;gap:7px;flex-wrap:wrap}.keys form{display:inline-block}.keys button{min-height:40px;padding:7px 10px;font-size:12px}.status{background:var(--surface);border:1px solid var(--line);padding:13px;margin-top:10px;color:#5f5d57;font-size:13px;line-height:1.45}.success{background:var(--okbg);border:1px solid #a9bea4;color:var(--ok);padding:16px;margin:14px 0}.success .btn{display:inline-flex;text-decoration:none;margin-top:10px}.muted{color:var(--muted)}details{margin-top:12px;color:var(--muted)}
...
</style></head><body><main class="wrap"><a class="back" href="/">← Volver al Publisher</a><div class="brandhead">${brand.logo_url?`<img src="${h(brand.logo_url)}" alt="">`:""}<strong>${h(show)}</strong></div>${brand.reference_image_url?`<img class="brandref" src="${h(brand.reference_image_url)}" alt="">`:""}<div class="eyebrow">GOOGLE FLOW · CONTROLADOR WEB</div><h1>Conectemos Google Flow.</h1><p class="lead">Este controlador funciona dentro del Publisher. Los botones ahora son acciones del servidor: cuando tocás <b>ABRIR GOOGLE</b>, la página se recarga y el navegador remoto aparece abajo. No depende de JavaScript del iPhone.</p>
...
<div class="live-note"><b>Control remoto completo:</b> ahora podés <b>scrollear con la rueda/trackpad</b> o <b>arrastrar verticalmente con el dedo</b> dentro de la pantalla. El navegador remoto usa una pantalla más grande para que entren más opciones.</div>
<div class="controller-tools"><a class="btn primary" href="/flow/bootstrap/view" target="_blank" rel="noopener">⛶ ABRIR PANTALLA COMPLETA</a><span class="controller-help">Si una opción queda fuera de vista, scrolleá directamente sobre el controlador. También tenés ↑ ↓ Home End dentro de la pantalla.</span></div>
...
<form method="post" action="/flow/bootstrap/key-view" target="flowController"><input type="hidden" name="key" value="Page_Up"><button type="submit">Page ↑</button></form>
<form method="post" action="/flow/bootstrap/key-view" target="flowController"><input type="hidden" name="key" value="Page_Down"><button type="submit">Page ↓</button></form>
<form method="post" action="/flow/bootstrap/key-view" target="flowController"><input type="hidden" name="key" value="Home"><button type="submit">Home</button></form>
<form method="post" action="/flow/bootstrap/key-view" target="flowController"><input type="hidden" name="key" value="End"><button type="submit">End</button></form>
...
if(!globalThis.__publisherFlowBootstrapInstalled){
 ...
 express.application.listen=function(...args){
  ...
  app.post('/flow/bootstrap/supabase-passkeys-page',async(_req,res)=>{try{await navigate(SUPABASE_PASSKEY_URL);go(res,'Supabase está abierto en el navegador remoto. Iniciá sesión con tu cuenta habitual y dejá abierta la página de Passkeys.')}catch(e){go(res,'',e?.message||e)}});
   ...
   app.get('/flow/bootstrap/screen.png',async(_req,res)=>{try{if(!pids().length||!windowId()){res.status(409).type('image/svg+xml').send('<svg xmlns="http://www.w3.org/2000/svg" width="'+VIEW.width+'" height="'+VIEW.height+'"><rect width="100%" height="100%" fill="#eee"/><text x="'+(VIEW.width/2)+'" y="'+(VIEW.height/2)+'" text-anchor="middle" font-family="Arial" font-size="30" fill="#555">Navegador cerrado</text></svg>');return}const png=shot();res.set('Cache-Control','no-store, max-age=0');res.type('png').send(png)}catch(e){res.status(500).type('text').send(compact(e?.message||e,500))}});
  app.get('/flow/bootstrap/view',(_req,res)=>{try{
    ...
    res.type('html').send(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no"><style>
*{box-sizing:border-box}html,body{margin:0;width:100%;min-height:100%;background:#111;overflow:auto}.stage{position:relative;width:min(100vw,${VIEW.width}px);aspect-ratio:${VIEW.width}/${VIEW.height};height:auto;margin:0 auto;background:#ececeb;touch-action:none;overscroll-behavior:contain}.shot{position:absolute;inset:0;width:100%;height:100%;object-fit:contain;user-select:none;-webkit-user-select:none}.top{z-index:2}.bottom{z-index:1}.busy{position:absolute;right:12px;top:12px;z-index:8;background:rgba(13,49,82,.88);color:#fff;padding:7px 9px;border-radius:8px;font:700 12px Arial;opacity:0;transition:opacity .15s;pointer-events:none}.busy.show{opacity:1}.scrollpad{position:absolute;right:10px;bottom:10px;z-index:9;display:grid;grid-template-columns:repeat(2,42px);gap:5px}.scrollpad button{width:42px;height:38px;border:1px solid rgba(255,255,255,.5);border-radius:8px;background:rgba(13,49,82,.82);color:#fff;font:700 16px Arial;box-shadow:0 2px 8px rgba(0,0,0,.22)}.scrollpad .wide{grid-column:1/-1;width:89px;font-size:10px}.hint{position:absolute;left:10px;bottom:10px;z-index:7;background:rgba(0,0,0,.66);color:#fff;padding:7px 9px;border-radius:7px;font:12px Arial;pointer-events:none}
</style></head><body><div class="stage" id="stage"><img id="a" class="shot top" src="/flow/bootstrap/screen.png?t=${Date.now()}" alt="Navegador remoto"><img id="b" class="shot bottom" alt=""><div id="busy" class="busy">actualizando…</div><div class="hint">Rueda/trackpad o deslizá para scrollear</div><div class="scrollpad"><button data-scroll="-620">↑</button><button data-scroll="620">↓</button><button class="wide" data-key="Home">HOME</button><button class="wide" data-key="End">END</button></div></div><script>
const VW=${VIEW.width},VH=${VIEW.height};
const stage=document.getElementById('stage'),a=document.getElementById('a'),b=document.getElementById('b'),busy=document.getElementById('busy');let front=a,back=b,actionBusy=false,refreshBusy=false,touchStart=null,lastWheel=0;
function coords(e){const r=stage.getBoundingClientRect();return{x:Math.max(0,Math.min(VW-1,(e.clientX-r.left)*VW/r.width)),y:Math.max(0,Math.min(VH-1,(e.clientY-r.top)*VH/r.height))}}
...
function refresh(){if(refreshBusy||actionBusy)return;refreshBusy=true;back.onload=()=>{swap();refreshBusy=false};back.onerror=()=>{refreshBusy=false};back.src='/flow/bootstrap/screen.png?t='+Date.now()}
async function post(url,payload){actionBusy=true;busy.classList.add('show');try{await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload||{})});setTimeout(refresh,90)}finally{setTimeout(()=>{actionBusy=false;busy.classList.remove('show')},180)}}
setInterval(refresh,700);
stage.addEventListener('click',async e=>{if(actionBusy||e.target.closest('.scrollpad'))return;const c=coords(e);await post('/flow/bootstrap/click',c)});
stage.addEventListener('wheel',e=>{e.preventDefault();if(actionBusy)return;const n=Date.now();if(n-lastWheel<45)return;lastWheel=n;const c=coords(e);post('/flow/bootstrap/scroll',{...c,deltaX:Math.max(-1200,Math.min(1200,e.deltaX*2.2)),deltaY:Math.max(-1600,Math.min(1600,e.deltaY*2.2))})},{passive:false});
stage.addEventListener('touchstart',e=>{if(e.touches.length!==1||e.target.closest('.scrollpad'))return;const t=e.touches[0],r=stage.getBoundingClientRect();touchStart={clientX:t.clientX,clientY:t.clientY,x:(t.clientX-r.left)*VW/r.width,y:(t.clientY-r.top)*VH/r.height}},{passive:true});
stage.addEventListener('touchmove',e=>{if(touchStart)e.preventDefault()},{passive:false});
stage.addEventListener('touchend',e=>{if(!touchStart||e.changedTouches.length!==1)return;const t=e.changedTouches[0],dx=t.clientX-touchStart.clientX,dy=t.clientY-touchStart.clientY,start=touchStart;touchStart=null;if(Math.abs(dy)>24||Math.abs(dx)>34){post('/flow/bootstrap/scroll',{x:start.x,y:start.y,deltaX:-dx*5,deltaY:-dy*5})}});
document.querySelectorAll('[data-scroll]').forEach(btn=>btn.addEventListener('click',e=>{e.stopPropagation();post('/flow/bootstrap/scroll',{x:VW/2,y:VH/2,deltaY:Number(btn.dataset.scroll)})}));
document.querySelectorAll('[data-key]').forEach(btn=>btn.addEventListener('click',e=>{e.stopPropagation();post('/flow/bootstrap/key',{key:btn.dataset.key})}));
</script></body></html>`);
  ...
  app.post('/flow/bootstrap/key-view',async(req,res)=>{try{
    await start();const key=String(req.body?.key||'');if(!['Tab','Return','BackSpace','Escape','Up','Down','Left','Right','Page_Up','Page_Down','Home','End'].includes(key))throw new Error('Tecla no permitida.');
  ...
  app.post('/flow/bootstrap/key-page',async(req,res)=>{try{await start();const key=String(req.body?.key||'');if(!['Tab','Return','BackSpace','Escape','Up','Down','Left','Right','Page_Up','Page_Down','Home','End'].includes(key))throw new Error('Tecla no permitida.');const w=windowId();if(w)spawnSync('xdotool',['windowactivate','--sync',w],{env:env(),timeout:3000});spawnSync('xdotool',['key','--clearmodifiers',key],{env:env(),timeout:3000});await sleep(200);go(res,'Tecla enviada: '+key)}catch(e){go(res,'',e?.message||e)}});
  app.post('/flow/bootstrap/finish-page',async(_req,res)=>{try{await verify();await stop();res.redirect('/?setup=complete')}catch(e){go(res,'',e?.message||e)}});
  ...
  app.post('/flow/bootstrap/scroll',async(req,res)=>{try{res.json(await scrollRemote({x:req.body?.x,y:req.body?.y,deltaX:req.body?.deltaX,deltaY:req.body?.deltaY}))}catch(e){safe(res,e)}});
  ...
  app.post('/flow/bootstrap/key',async(req,res)=>{try{await start();const key=String(req.body?.key||'');if(!['Tab','Return','BackSpace','Escape','Up','Down','Left','Right','Page_Up','Page_Down','Home','End'].includes(key))throw new Error('Tecla no permitida.');spawnSync('xdotool',['key','--clearmodifiers',key],{env:env(),timeout:3000});res.json({ok:true})}catch(e){safe(res,e)}});
