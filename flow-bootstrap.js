...
${running?`<div class="status"><b>Navegador activo.</b> ${h(title||'Google')}<br><span class="muted">Si la pantalla no cambió, tocá ACTUALIZAR NAVEGADOR.</span></div>
<form method="post" action="/flow/bootstrap/click-page"><div class="screen-wrap"><input class="screen-input" type="image" name="screen" src="/flow/bootstrap/screen.png?t=${Date.now()}" width="1024" height="700" alt="Navegador remoto de Google Flow"></div></form>
<p class="screen-note">La captura tiene 1024 × 700 px. Tocarla envía ese clic al Chrome remoto.</p>
<form class="entry" method="post" action="/flow/bootstrap/type-page"><input name="text" type="password" autocomplete="off" placeholder="Primero tocá un campo en la captura y después escribí acá"><button class="primary" type="submit">ESCRIBIR Y BORRAR</button></form>
<details><summary>Controles extra</summary><div class="keys">
<form method="post" action="/flow/bootstrap/key-page"><input type="hidden" name="key" value="Tab"><button type="submit">Tab</button></form>
<form method="post" action="/flow/bootstrap/key-page"><input type="hidden" name="key" value="Return"><button type="submit">Enter</button></form>
<form method="post" action="/flow/bootstrap/key-page"><input type="hidden" name="key" value="BackSpace"><button type="submit">Backspace</button></form>
<form method="post" action="/flow/bootstrap/key-page"><input type="hidden" name="key" value="Escape"><button type="submit">Escape</button></form>
...
if(!globalThis.__publisherFlowBootstrapInstalled){
 ...
 express.application.listen=function(...args){
  ...
  app.post('/flow/bootstrap/finish-page',async(_req,res)=>{try{const out=await verify();await stop();go(res,'Google Flow conectado ✓ · '+(out.project_name||out.project_id||'proyecto detectado'));setTimeout(()=>process.exit(0),1800).unref?.()}catch(e){go(res,'',e?.message||e)}});
