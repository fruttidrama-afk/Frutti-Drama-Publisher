...
const providerSchemas=(schema.properties?.publication?.properties?.providers?.items?.oneOf||[]);
const ytSchema=providerSchemas.find(x=>x?.properties?.type?.const==='youtube');
...
must(publication.includes('sourceMediaTransferred=true'),'YouTube approval must transfer the existing Review file');
must(facebookPublication.includes('sourceMediaTransferred=true'),'Facebook approval must transfer the existing Review file');
must(facebookPublication.includes('is_ai_generated:true'),'Facebook Reels must publish with native AI disclosure');
must(facebookPublication.includes('ensureAiDisclosure'),'Facebook published-Reel AI disclosure repair must exist');
must(facebookPublication.includes("params:{is_ai_generated:true}"),'Facebook existing Reel must be repairable in place without re-upload');
must(!publication.includes('fs.copyFileSync(row.videoPath,filePath)'),'same-volume Review copy must remain forbidden for YouTube');
must(!facebookPublication.includes('fs.copyFileSync(row.videoPath,filePath)'),'same-volume Review copy must remain forbidden for Facebook');
must(server.includes('[APPROVAL MEDIA HANDOFF]'),'approval storage handoff observability missing');
